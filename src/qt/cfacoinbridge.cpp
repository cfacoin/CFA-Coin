#include "cfacoinbridge.h"

#include "cfacoingui.h"
#include "guiutil.h"
...
public:
    ...
    QString addMessage(int row)
    {
        return QString("{\"id\":\"%10\",\"type\":\"%1\",\"sent_date\":\"%2\",\"received_date\":\"%3\", \"label_value\":\"%4\",\"label\":\"%5\",\"to_address\":\"%6\",\"from_address\":\"%7\",\"message\":\"%8\",\"read\":%9},")
                ...
                .arg(mtm->index(row, MessageModel::Message)         .data().toString().replace("\\","\\\\").replace("\"","\\\"").replace("\n", "\\n"))
...
#include "cfacoinbridge.moc"

CFACoinBridge::CFACoinBridge(CFACoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void CFACoinBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void CFACoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void CFACoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void CFACoinBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void CFACoinBridge::paste() {
    emitPaste(QApplication::clipboard()->text());
}

// Options
void CFACoinBridge::populateOptions()
{
...
// Transactions
bool CFACoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void CFACoinBridge::clearRecipients()
{
    recipients.clear();
}

bool CFACoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 OK, 1 CFACoin
        switch(rcp.txnTypeInd)
        {
            case TXT_OK_TO_OK:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CFA, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_OK_TO_ANON:
                formatted.append(tr("<b>%1</b> to CFACOIN %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CFA, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to CFACOIN %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CFA, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_OK:
                formatted.append(tr("<b>%1</b>, ring size %2 to CFA %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CFA, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CFA, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsCFACoinBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your cfacoin balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::CFA, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void CFACoinBridge::openAddressBook(bool sending)
{
...
void CFACoinBridge::openCoinControl()
{
...
void CFACoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void CFACoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap CFACoinBridge::listAnonOutputs()
{
...
void CFACoinBridge::populateTransactionTable()
{
...
void CFACoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void CFACoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString CFACoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void CFACoinBridge::populateAddressTable()
{
...
void CFACoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void CFACoinBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString CFACoinBridge::newAddress(bool own)
{
...
QString CFACoinBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void CFACoinBridge::updateAddressLabel(QString address, QString label)
{
...
bool CFACoinBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool CFACoinBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void CFACoinBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void CFACoinBridge::appendMessage(int row) {
    emitMessage(window->messageModel->index(row, MessageModel::Key)             .data().toString(),
                ...
                window->messageModel->index(row, MessageModel::Message)         .data().toString());
}

void CFACoinBridge::populateMessageTable()
{
...
void CFACoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool CFACoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool CFACoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString CFACoinBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool CFACoinBridge::setPubKey(QString address, QString pubkey)
{
...
bool CFACoinBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
void CFACoinBridge::connectSignals()
{
...
QVariantMap CFACoinBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap CFACoinBridge::listLatestBlocks()
{
...
QVariantMap CFACoinBridge::findBlock(QString searchID)
{
...
QVariantMap CFACoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap CFACoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap CFACoinBridge::txnDetails(QString blkHash, QString txnHash)
{
