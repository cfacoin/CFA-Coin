...
var overviewPage = {
    init: function() {
        ...
        // Announcement feed
        $.ajax({
            url:"https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&q=http://updates.cfacoin.co/rss",
            dataType: 'jsonp'
    ...
    updateBalance: function(balance, cfacoinBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            cfacoinBal   = this.cfacoinBal  .data("orig"),
            ...
            this.cfacoinBal  .data("orig", cfacoinBal),
        ...
        this.formatValue("cfacoinBal",   cfacoinBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. FzjjetSdTwrppUwwNUo1GFHYTibzJi77jM)" placeholder="Enter an CFACoin address (e.g. FzjjetSdTwrppUwwNUo1GFHYTibzJi77jM)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="CFACoin"                    ' + (unit.type == 0 ? "selected" : "") + '>CFA</option> \
                    <option value="1" title="Milli-CFACoin (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mCFA</option> \
                    <option value="2" title="Micro-CFACoin (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;CFA</option> \
                    <option value="3" title="Nano-CFACoins (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>nCFA</option> \
                </select> \
            </div> \
