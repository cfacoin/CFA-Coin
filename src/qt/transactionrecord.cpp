...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return CFACoinGUI::tr("Received with");
    case RecvFromOther:
        return CFACoinGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return CFACoinGUI::tr("Sent to");
    case SendToSelf:
        return CFACoinGUI::tr("Payment to yourself");
    case Generated:
        return CFACoinGUI::tr("Mined");
    case RecvCFACoin:
        return CFACoinGUI::tr("Received cfacoin");
    case SendCFACoin:
        return CFACoinGUI::tr("Sent cfacoin");
    case Other:
        return CFACoinGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvCFACoin:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendCFACoin:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredCFA, nCredCFACoin;
    wtx.GetCredit(nCredCFA, nCredCFACoin, true);
    int64_t nCredit = nCredCFA + nCredCFACoin;
    ...
    {
        if (nNet > 0 && nCredCFACoin > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvCFACoin, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendCFACoin, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvCFACoin;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv cfacoin
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendCFACoin;
                    sub.address = CBitcoinAddress(ckidD).ToString();
