// Copyright (c) 2018 The CFAcoin Developers
...
CFACoinGUI::CFACoinGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new CFACoinBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
...

{
    ...
    resize(1280, 720);
    setWindowTitle(tr("CFACoin") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/cfacoin"));
    setWindowIcon(QIcon(":icons/cfacoin"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/cfacoin/index.html");
#else
    QFile html("/opt/cfacoin/index.html");
#endif
...
void CFACoinGUI::pageLoaded(bool ok)
{
...
void CFACoinGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);
}

void CFACoinGUI::urlClicked(const QUrl & link)
{
...
void CFACoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("CFACoin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/cfacoin_testnet"));
            setWindowIcon(QIcon(":icons/cfacoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/cfacoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("CFACoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/cfacoin_testnet"));
...
void CFACoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void CFACoinGUI::setMessageModel(MessageModel *messageModel)
{
...
void CFACoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setIcon(QIcon(":/icons/cfacoin"));
...
#ifndef Q_OS_MAC
void CFACoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CFACoinGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to CFACoin network", "", count));
}

void CFACoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void CFACoinGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void CFACoinGUI::changeEvent(QEvent *e)
{
...
void CFACoinGUI::closeEvent(QCloseEvent *event)
{
...
void CFACoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::CFA, nFeeRequired));
...
void CFACoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void CFACoinGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void CFACoinGUI::gotoSignMessageTab(QString addr)
{
...
void CFACoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void CFACoinGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void CFACoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CFACoinGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void CFACoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
void CFACoinGUI::handleURI(QString strURI)
{
...
void CFACoinGUI::setEncryptionStatus(int status)
{
...
void CFACoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void CFACoinGUI::changePassphrase()
{
...
void CFACoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void CFACoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CFACoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void CFACoinGUI::updateStakingIcon()
{
...
void CFACoinGUI::detectShutdown()
{
