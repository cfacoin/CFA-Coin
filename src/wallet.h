...
public:
    ...
    int64_t GetCFACoinBalance() const;
    ...
    int64_t GetCFACoinDebit(const CTxIn& txin) const;
    int64_t GetCFACoinCredit(const CTxOut& txout) const;
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetCFACoinCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nCFA, int64_t& nCFACoin) const
    {
        nCFA = 0;
        nCFACoin = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCFACoin += GetCFACoinCredit(txout);
            } else
                nCFA += GetCredit(txout);
            if (!MoneyRange(nCFA)
                || !MoneyRange(nCFACoin))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableCFACoinCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableCFACoinCreditCached;
    
    mutable int64_t nCredCFACached;
    mutable int64_t nCredCFACoinCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableCFACoinCreditCached = false;
        
        nCredCFACached = 0;
        nCredCFACoinCached = 0;
        ...
        nAvailableCFACoinCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredCFA, int64_t& nCredCFACoin, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredCFA = nCredCFACoin = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredCFACached = 0;
            nCredCFACoinCached = 0;
            pwallet->GetCredit(*this, nCredCFACached, nCredCFACoinCached);
            fCreditSplitCached = true;
        };
        
        nCredCFA = nCredCFACached;
        nCredCFACoin = nCredCFACoinCached;
        return true;
    }
    ...
    int64_t GetAvailableCFACoinCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableCFACoinCreditCached)
            return nAvailableCFACoinCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableCFACoinCredit() : value out of range");
            };
        };
        
        nAvailableCFACoinCreditCached = nCredit;
        fAvailableCFACoinCreditCached = true;
        return nCredit;
    };
